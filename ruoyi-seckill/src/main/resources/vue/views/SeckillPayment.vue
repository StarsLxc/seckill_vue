      ...
      <el-col :span="24" :xs="24">
        ...
        <el-form ref="queryForm" :model="queryParams" :inline="true" label-width="100px">
          <!-- 数据筛选表单组件 -->
            ...
            <el-input v-model="queryParams.paymentAmount" placehold clearable size="small" @keyup.enter="handleQuery" />
          </el-form-item>
                  <el-form-item label="支付状态" prop="paymentStatus">
            <el-input v-model="queryParams.paymentStatus" placehold clearable size="small" @keyup.enter="handleQuery" />
          </el-form-item>
                  <el-form-item label="支付时间" prop="paymentTime">
            <el-input v-model="queryParams.paymentTime" placehold clearable size="small" @keyup.enter="handleQuery" />
          </el-form-item>
                  <el-form-item label="支付方式" prop="paymentType">
            <el-input v-model="queryParams.paymentType" placehold clearable size="small" @keyup.enter="handleQuery" />
          </el-form-item>
        ...
        <!-- 数据表格 -->
        <el-table v-loading="loading" :data="seckillPaymentList">
          <!-- 数据表格列 -->
              <el-table-column label="账单id" align="center" prop="paymentId" />
...
<script>
/** 导入axios Api */
import { listSeckillPaymentByPage, getSeckillPayment, addSeckillPayment, updateSeckillPayment, deleteSeckillPayment } from '@/api/seckill/payment'
export default {
  name: 'SeckillPayment',
  data() {
    return {
  ...
  methods: {
    /** 查询SeckillPayment数据列表 */
    getList() {
      this.loading = true
      listSeckillPaymentByPage(this.queryParams).then((response) => {
        this.seckillPaymentList = response.data.list
    ...
    handleAdd() {
      this.resetForm()
      this.open = true
      this.title = '添加SeckillPayment'
    },

    /** 修改按钮操作 */
    handleUpdate(row) {
      this.resetForm()
      getSeckillPayment(row.paymentId).then((response) => {
        this.form = response.data
        this.open = true
        this.title = '修改SeckillPayment'
      })
    },
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          if (this.form.paymentId !== null) {
            updateSeckillPayment(this.form).then((response) => {
              this.open = false
              this.title = ''
              this.getList()
            ...
            addSeckillPayment(this.form).then((response) => {
              this.open = false
              this.title = ''
              this.getList()
              this.$message.success(response.data)
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$confirm('是否确认删除ID为"' + row.paymentId + '"的数据项?', '警告', {
          confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'
        })
        .then(function() {
          return deleteSeckillPayment(row.paymentId)
        })
        .then(() => {
          this.getList()
